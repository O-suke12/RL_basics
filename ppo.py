...
def parse_args():
    ...
    parser.add_argument("--num-episodes", type=int, default=100, help="the number of episodes to run")
    parser.add_argument(
...
class PPO:
    ...
    def roll_out(self):
        ...
        rtgs = torch.FloatTensor(self.to_go(ep_rews))  # rtgs: nd_array, log_probs: torch_tensor
        return (
            obs,
            acts,
            rtgs,
    ...
    def train(self):
        for epi in range(args.num_episodes):
            ...
            for upd in range(args.num_updates):
                total_upd = epi * args.num_updates + upd
                ...
                writer.add_scalar("loss-actor", actor_loss, total_upd)
                writer.add_scalar("loss-critic", critic_loss, total_upd)
            writer.add_scalar("epi-length", len(rtgs), total_upd)
...
if __name__ == "__main__":
    ...
    # env = gym.wrappers.RecordVideo(env, "video", episode_trigger=lambda x: x % 20 == 0)
    ...
    if args.render:
        obs = env.reset()[0]
        action = agent.get_action_and_prob(obs)[0]
        while True:
